    ...
    function getMsg() {
        ...
        if (typeof (EventSource) !== 'undefined') {
            ...
            serverUpdate.onmessage = function (event) {
                ...
                $('#totalAssHeader').html("You have " + totalAssCount + " New Payment Requests!");
                $('#ass-menu-id').html('');
                for (var i in data[0]) {
                    ...
                    $('#ass-menu-id').append('<li><a href="paymentorder.php">' + data[0][i].fname + ' ' + data[0][i].mname + ' :  '+data[0][i].testcatagory+'</div>');
                }
            ...
            alert("Your browser is very old please use modern browser");
        }
    }
...
function getUserProfile(id) {
    ...
    $.ajax({
        ...
        error: function (errors) {
            alert("errorThrown User not found");
        }
    });
}