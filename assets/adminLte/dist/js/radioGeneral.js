    ...
    function getMsg() {
        if (typeof (EventSource) !== 'undefined') {
            ...
            serverUpdate.onmessage = function (event) {
                ...
                $('#totalAssHeader').html("You have " + totalAssCount + " Orders");
                $('#ass-menu-id').html('');
                for (var i in data[0]) {
                    ...
                    $('#ass-menu-id').append('<li><a href="radiologywindow.php">' + data[0][i].fname + ' ' + data[0][i].mname +' '+data[0][i].lname+'-'+data[0][i].testcatagory+ '</div>');
                }
