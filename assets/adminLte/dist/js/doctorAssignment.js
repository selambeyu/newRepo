    ...
    function getMsg() {
        if (typeof (EventSource) !== 'undefined') {
            ...
            serverUpdate.onmessage = function (event) {
                ...
                $('#totalAssHeader').html("You have " + totalAssCount + " Waiting Patient!");
                $('#ass-menu-id').html('');

                for (var i in data[0]) {
                    if(data[0][i].ptype=='Antenatal'){
                       $('#ass-menu-id').append('<li><a href="antenatalTreatment.php?id=' + data[0][i].cardNo + ' &assId=' + data[0][i].aid + '">' + data[0][i].fname + ' ' + data[0][i].mname + ' ' + data[0][i].lname + '</div>');
    
                    }
                    else{
                        $('#ass-menu-id').append('<li><a href="treatment.php?id=' + data[0][i].cardNo + ' &assId=' + data[0][i].aid + '">' + data[0][i].fname + ' ' + data[0][i].mname + ' ' + data[0][i].lname + '</div>');
                    }
                ...
                $('#messages-menu-id').html('');
                for (var i in data[1]) {
                    $('#messages-menu-id').append('<li> <a href="#">' + data[1][i].fname + ' '+ data[1][i].mname +':  ' + data[1][i].msgBody + '</a></li>');
                }
